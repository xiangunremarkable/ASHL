...
class ParameterServer(nn.Module):
    ...
    def BSP(self, args, dic):
        ...
        if self.flag:
            ...
            for g, p in zip(mean_list, self.model.named_parameters()):
                if g is not None:
                    p[1].grad = g.cpu()
            self.optimizer.step()
            self.mean_tensor_list={k: v.cpu() for k, v in self.model.state_dict().items() if 'weight' in k or 'bias' in k}
            if step % endflag == 0:
                self.scheduler.step()
        lock.release()
        ...
        return time5, self.continue_flag, self.mean_tensor_list
    ...
    def SSP(self, args, dic):
        ...
        if step % endflag == 0:
            ...
            self.ps_time[client_id] = 0
        time5 = time.time() - time1
        return time5, self.continue_flag, tensor_list
    ...
    def Local(self, args, dic):
        ...
        if self.flag:
            ...
            self.model.load_state_dict(self.mean_tensor_list,False)
        lock.release()
    ...
    def test(self):
        ...
        self.test_num += 1
        lock.acquire()
        param_rrefs = {k: v for k, v in self.model.state_dict().items()}
...
if __name__ == '__main__':
    ...
    if args.rank == 0:
        ...
        test_loader = torch.utils.data.DataLoader(
            testset, batch_size=500, shuffle=False,num_workers=data_worker)
