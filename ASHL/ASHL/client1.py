...
class Trainer(nn.Module):
    def __init__(self):
        ...
        warm_up_with_cosine_lr = lambda epoch: epoch / warmup_epoch if epoch <= warmup_epoch else 0.5 * (
                math.cos((epoch - warmup_epoch) / (max_epoch - warmup_epoch) * math.pi) + 1)
        cosine_lr = lambda epoch:  0.5 * (math.cos((epoch - warmup_epoch) / (max_epoch - warmup_epoch) * math.pi) + 1)
    ...
    def get_part(self):
        ...
        # time_ma[0] = iter_time 
        time_list = [dic['client'], sum(self.monitor)/len(self.monitor), sum(self.monitor1)*4/len(self.monitor1), sum(self.monitor2)*4/len(self.monitor2), sum(self.monitor3)/len(self.monitor3)]
...
def run_training_loop(rank, n_epochs, train_loader, test_loader):
    ...
    for i in range(len(net.monitor)):
        str_log = str(net.monitor[i]) + ',' + str(net.monitor1[i]) + ',' + str(net.monitor2[i])+ ',' + str(net.monitor3[i])
    ...
    while(net.flag and i2<epoch2):
        ...
        for data, target in train_loader:
            ...
            if i % net.H == 0 and net.flag and not net.change_flag:
                ...
                if i1%6==0:
                    net.scheduler.step()
                time.sleep((time.time() - time3 -net.ps_time-net.compress_time)*3)
