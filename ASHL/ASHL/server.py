...
parser.add_argument("--learning_rate3", type=float, default=0.3)
...
class ParameterServer(nn.Module):
    ...
    def get_h(self):
        ...
        list2 = [self.m_dict1[i]  for i in range(self.wait_client_sc)]
        list3 = [self.m_dict2[i]  for i in range(self.wait_client_sc)]
        list4 = [self.cm_dict[i]  for i in range(self.wait_client_sc)]
        list5 = [self.m_dict1[i] + self.m_dict2[i]  for i in range(self.wait_client_sc)]
        h_list = []
        h_list1 = []
        p_list = []
        sum_list = [0 for i in range(n)]
        pre_cp = compress_ratio  
        cp_list = [1.0 for i in range(self.wait_client_sc)]
        list6=[list2[i]* pre_cp + list3[i] + list4[i]  for i in range(self.wait_client_sc)]
        # x_max = max(list6)
        # for i in range(self.wait_client_sc):
        #     if list2[i]+ list3[i] + list4[i]>x_max:
        #         cp_list[i] =(x_max-list3[i]-list4[i])/list2[i] 
        #         if cp_list[i]>1.0:
        #             cp_list[i]=1.0
        # list6=[list2[i]* cp_list[i] + list3[i] + list4[i]  for i in range(self.wait_client_sc)]
        k=2
        list7 = [list1[i]*k +list6[i] for i in range(self.wait_client_sc)]
        x_max = max(list7)
        h_list2 = [0 for i in range(n)]
        for j in range(0, n):
            h_list2[j] = round((x_max-list6[j])/list1[j])
        ...
        list7 = [list1[i]*h_list1[i] +list6[i] for i in range(self.wait_client_sc)]
        list6 =[list1[i]*h_list1[i]+ list3[i] + list4[i]  for i in range(self.wait_client_sc)]
        m_T = max(list7)
        for i in range(self.wait_client_sc):
            if (m_T-list6[i])/list2[i]<=1.0:
                cp_list[i] =(m_T-list6[i])/list2[i] 
        sum_h=sum(h_list1)
        part=[float(h_list1[i])/sum_h for i in range(self.wait_client_sc)]
        part=[round(part[i],3)for i in range(self.wait_client_sc)]
        k=8
        list7 = [list1[i]*k +list5[i] for i in range(self.wait_client_sc)]
        x_max = max(list7)
        h_list2 = [0 for i in range(n)]
        for j in range(0, n):
            h_list2[j] = round((x_max-list5[j])/list1[j])
            sum_list[j] = h_list2[j]*list1[j]+list2[j]
        h_list1 =[k for k in h_list2]
        h_list.append(h_list1)
        print(h_list)
    ...
    def get_part(self, args):
        ...
        self.m_dict1[client_id] = args[2]
        self.m_dict2[client_id] = args[3]
        self.cm_dict[client_id] = args[4]
        while self.count < self.wait_client_sc:
            time.sleep(0.01)
        lock.acquire()
        if self.flag:
            self.flag = False
            self.H ,self.part ,self.cplist = self.get_h()
        lock.release()
        return self.H ,self.part ,self.cplist 
    ...
    def SLocal(self, args, dic):
        ...
        if self.finish_node!=0 or not self.continue_flag:
            ...
            flag1 = False
            time1 = time.time()-time1
            return time1,w_avg,flag1,self.change_flag       
        ...
        return time5, global_para,flag1,self.change_flag
    ...
    def test(self): 
        ...
        if not self.change_flag and train_loss<learning_rate3:
    ...
    def finish(self):
        ...
        if self.finish_node == self.wait_client_sc:
            f = './log/' + str(train_type) +  "_" + str(epoch2) + "_"+ str(compress_ratio)+ "_"+ str(learning_rate2) + "_"+ str(learning_rate3) + "_" + "sever.csv"
            with open(f, "a") as file:  # 只需要将之前的”w"改为“a"即可，代表追加内容
            ...
            f = './log/' + str(train_type) + "_" + str(epoch2) + "_"+ str(compress_ratio)+ "_"+ str(learning_rate2) + "_"+ str(learning_rate3) + "_" + str(nowTime) + ".csv"
            with open(f, "a") as file:  # 只需要将之前的”w"改为“a"即可，代表追加内容
                file.write('client_id' + ',' + 'time' + ',' + 'state' + "\n")
                for key in self.monitor:
                    file.write(str(key) + "\n")
                file.close()
            f = './log/' + str(train_type) + "_" + str(epoch2) + "_"+ str(compress_ratio)+ "_"+ str(learning_rate2) + "_"+ str(learning_rate3) + "_" + "train_sever" + ".csv"
            with open(f, "a") as file:  # 只需要将之前的”w"改为“a"即可，代表追加内容
                file.write('test_num' + ',' + 'nowTime' + ',' + 'para_time'+ ',' + 'train_time' + ',' + 'train_loss' + ',' + 'acc' + "\n")
                for key in self.monitor1:
                    file.write(str(key) + "\n")
                file.close()
            f = './log/' + str(train_type) + "_" + str(epoch2) + "_"+ str(compress_ratio)+ "_"+ str(learning_rate2) + "_"+ str(learning_rate3) + "_" + "test_sever" + ".csv"
            with open(f, "a") as file:  # 只需要将之前的”w"改为“a"即可，代表追加内容
...
def partition_dataset(partition_sizes):  # 数据分割
    ...
    train_set = torch.utils.data.DataLoader(partition, batch_size=batch_size, shuffle=True)  # 分割的数据加载成纯set
...
class Trainer(nn.Module):
    ...
    def sync(self):
        ...
        self.ps_time, remote_params = remote_method(
    ...
    def sync1(self):
        ...
        sparsify_time += time.time() - time1
        #tensor_list = {k: v.cpu() for k, v in self.model.state_dict().items() if 'weight' in k or 'bias' in k}
        
        remote_params, flag,self.ps_time = remote_method(
            ParameterServer.Local1,
            self.param_server_rref,
            tensors,
            dic
        )
    ...
    def pre_sync(self):
        ...
        time1=time.time()
        self.pre_num+=1
        dic1 = self.model.state_dict()    #参数提取
        self.model.load_state_dict(dic1)  #更新参数（提前）
        # iter_time += time.time()-time1
        # time_ma[0] = iter_time            #总计算时间
        arg = []
        arg.append(dic['client'])
        arg.append(dic['step'])
        arg.append(time1)
        ...
        time_ma[1] += remote_params[0] 
        time_ma[2] += ( time.time() - remote_params[1])

    def get_part(self):
        global iter_time
        time_ma[0] = iter_time 
        time_list = [dic['client'], time_ma[0]/self.pre_num, time_ma[1]/self.pre_num, time_ma[2]/self.pre_num]
        h, part= remote_method(
...
def run_training_loop(rank, n_epochs, train_loader, test_loader):
    ...
    for epoch in range(0, n_epochs):
        ...
        total = 0
        # if epoch == 0:
        #     net.H = net.h[4][rank-1]
        #     net.compress_ratio=1.0
        # elif epoch == 40:
        #     net.H = net.h[3][rank-1]
        #     net.compress_ratio=0.8
        # elif epoch == 80:
        #     net.H = net.h[2][rank-1] 
        #     net.compress_ratio=0.6
        # elif epoch == 120:
        #     net.H = net.h[1][rank-1]
        #     net.compress_ratio=0.4
        # elif epoch == 160:
        #     net.H = net.h[0][rank-1] 
        #     net.compress_ratio=0.2   
        if  not net.flag:
            break  
        time1 = time.time()
        for data, target in train_loader:
            ...
            total += target.size(0)
            correct += predicted.eq(target).sum().item()
            time.sleep((time.time() - time2))
            iter_time += (time.time() - time2)
            if i % net.H == 0 and net.flag:
                i1+=1
                dic['step'] = i1
                time3 = time.time()
                net.sync1()
                time.sleep((time.time() - time3 -net.ps_time))
    ...
    f = './log/' + str(train_type) + "_" + "test"+ "_" + str(learning_rate) + "_" + str(rank) + ".csv"
    with open(f, "a") as file:  # 只需要将之前的”w"改为“a"即可，代表追加内容
        file.write('epoch' + ',' + 'nowTime' + ',' + 'test_time' + ',' + 'test_loss' + ',' + 'acc' + "\n")
        for key in monitor:
            file.write(str(key) + "\n")
        file.close()
    f = './log/' + str(train_type) + "_" + "train" + "_"+ str(learning_rate) + "_" + str(rank) + ".csv"
    with open(f, "a") as file:  # 只需要将之前的”w"改为“a"即可，代表追加内容
        file.write(
