...
class ParameterServer(nn.Module):
    ...
    def reward(self, arg):
        ...
        if self.finish_node>0:
            ...
            self.monitor.append(str_log)
            time2 = time.time() - time1
            return False,False,time2
        lock1.acquire()
        if self.flag == 0:
            self.flag = 1
            #self.scheduler.step()
            max_c = [value for kay, value in self.client_r.items()]
            next_f =[value for kay, value in self.r_next.items()]
            a= next_f.count(False)
        ...
        self.monitor.append(str_log)
        time2 = time.time() - time1
        return self.reward,self.able_flag,time2
    ...
    def ASP(self, args, dic):
        ...
        lock2.acquire()
        self.waiting_time[client_id] = (time.time()-time1)
        ...
        a = client_id + 1
        str_log = str(a) + ',' + str(self.all_time[client_id]) + ',' + str(self.waiting_time[client_id])
...
class Trainer(nn.Module):
    ...
    def reward(self):
        ...
        # print(rank,"reward out")
        str_log="reward out"+','+str(remote_params)
        write_log3(str_log, rank)
    ...
    def train(self):
        ...
        while time.time()-time1<self.T:
            ...
            time.sleep((time.time() - time3-self.pstime)*3)
            self.comm_time+=(time.time() - time3-self.pstime)
            self.T1= time.time()-time3    
